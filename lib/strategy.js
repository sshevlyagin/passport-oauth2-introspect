...
Strategy.prototype._introspect = function _introspect (token, callback) {
  ...
  self._cache.get(token, function (err, cachedData) {
    ...
    if (cached_token && cached_token.active !== undefined) {
      ...
      // Introspect the token using axios
      axios.post(self._introspect_url, new URLSearchParams({ token: token }), {
        auth: {
        ...
        httpsAgent: new (require('https').Agent)({ rejectUnauthorized: self._strictSSL }),
      })
      .then(function(response) {        
        self._introspectCallback(callback, token, null, response, response.data);
      })
      .catch(function(error) {        
        self._introspectCallback(
          callback,
          token,
          error,
          error.response,
